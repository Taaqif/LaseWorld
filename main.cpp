...
bool StageMenu = false;
bool ControlMenu = false;
bool MainMenu = true;
//rollover button default states
bool stage1Rollover = false;
bool stage2Rollover = false;
bool stage3Rollover = false;
bool controlRollover = false;
bool stageRollover = false;
bool playRollover = false;
bool scoreRollover = false;
bool exitRollover = false;
bool scoreMainMenuRollover = false;
bool controlMainMenuRollover = false;
bool stageMainMenuRollover = false;
int Armour = 100;
int score = 0;
...
int frameCount = 0;
clock_t lastClock = 0;
///obj map for string obj files loaded 
std::map<std::string,ModelLoader*> obj;
int currentLevel = 0;
//music
CEasySound *es;
CSound* bgmusic;
...
void RolloverMouse(int x, int y);
...
void LightingInit(){
	...
	GLfloat lightpos[] = { 2636.0 - 17290, 29900.0,
	42032.0, 0 };
...
void drawLoadingScreen(std::string msg, int percentage) {
	//char defaultMessage[] = "Loading";
	...
	glPushMatrix();
		...
		glPushMatrix();       // 2d modelview
			...
			glPushMatrix();
				...
				glRotatef(180, 1, 0, 0);
				//for each character in string loading
				for (std::string::size_type i = 0; i < loading.size(); ++i) {
					glutStrokeCharacter(GLUT_STROKE_ROMAN, loading[i]);
				}
			glPopMatrix();
			...
			glPushMatrix();
				...
				glRotatef(180, 1, 0, 0);
				//for each character in string msg
			...
			glPushMatrix();
				...
				glBegin(GL_QUADS);
					glVertex2f(0.0, 0.0);
					glVertex2f(percentage*6.5, 0.0);
					glVertex2f(percentage*6.5, 10.0);
					glVertex2f(0.0, 10.0);
				glEnd();
			glPopMatrix();
...
//BEN FIRING ROBOT FUNCTION


void FireRobot(int val){

	for (int i = 0; i < 30; i++){
		if (BasicEnemyArray[i].Alive){
	...
	if (inCombat){
		glutTimerFunc(TIMERTICKFIRE, FireRobot, val);
	}
	
}


//BEN CREATES ROBOT OBJECTS
bool StartBasicEnemy = false;
bool tempval = true;
void CreateRobots(){

	// default position GLint RobotPosX = 1000, RobotPosY = 11000, RobotPosZ = 40000, RobotWidthX = 100, RobotWidthZ = 100, RobotHeight = 500;
	// format GetRobotPos(GLuint xpos, GLuint ypos, GLuint zpos);
	if (StartBasicEnemy){
		for (int i = 0; i < 30; i++){
			BasicEnemyArray[i] = Robot();
		}
		StartBasicEnemy = false;
		BasicEnemyArray[20].SetEnemyHP(3);
		BasicEnemyArray[21].SetEnemyHP(3);
		BasicEnemyArray[22].SetEnemyHP(3);
		BasicEnemyArray[23].SetEnemyHP(3);
	}

	
	
	if (progFlag == 2){
		BasicEnemyArray[1].SetRobotPos(500, 10400, 40000, 'f', 150, 100, 400, "TestTargetWarrior");
		BasicEnemyArray[2].SetRobotPos(-2000, 10400, 35000, 'f', 150, 100, 400, "TestTargetWarrior");
		BasicEnemyArray[3].SetRobotPos(-3000, 10400, 38000, 'f', 150, 100, 400, "TestTargetWarrior");
		BasicEnemyArray[4].SetRobotPos(-4000, 11400, 34000, 'f', 150, 200, 100, "TestTargetObserver");
		BasicEnemyArray[5].SetRobotPos(-4000, 11400, 34000, 'f', 150, 200, 100, "TestTargetObserver");
	}
	//Stage 2
	if (progFlag == 18){
		//stage 2 at the first door door
		BasicEnemyArray[6].SetRobotPos(2000, 11500, 37000, 'f', 150, 100, 400, "WarriorFrontStanding");
		BasicEnemyArray[7].SetRobotPos(1800, 11500, 35000, 'f', 150, 100, 400, "WarriorFrontStanding");
		BasicEnemyArray[8].SetRobotPos(2000, 11500, 33600, 'f', 150, 100, 400, "WarriorFrontStanding");
		BasicEnemyArray[9].SetRobotPos(1900, 11500, 34000, 'f', 150, 100, 400, "WarriorFrontStanding");
		BasicEnemyArray[10].SetRobotPos(2300, 11500, 37000, 'f', 150, 100, 400, "WarriorFrontStanding");
		BasicEnemyArray[6].SetEnemyAlive(true);
		BasicEnemyArray[7].SetEnemyAlive(true);
		BasicEnemyArray[8].SetEnemyAlive(true);
		BasicEnemyArray[9].SetEnemyAlive(true);
		BasicEnemyArray[10].SetEnemyAlive(true);
	}
	if (progFlag == 23){
		//second cover point
		BasicEnemyArray[11].SetRobotPos(-6000, 11500, 32500, 'r', 150, 100, 400, "WarriorFrontStanding");
		BasicEnemyArray[12].SetRobotPos(-6000, 11500, 32800, 'r', 150, 100, 400, "WarriorFrontStanding");
		BasicEnemyArray[13].SetRobotPos(-5000, 11500, 33100, 'r', 150, 100, 400, "WarriorFrontStanding");
		BasicEnemyArray[14].SetRobotPos(-4000, 11500, 33000, 'r', 150, 100, 400, "WarriorFrontStanding");
		BasicEnemyArray[15].SetRobotPos(-5500, 11500, 32500, 'r', 150, 100, 400, "WarriorFrontStanding");
		BasicEnemyArray[11].SetEnemyAlive(true);
		BasicEnemyArray[12].SetEnemyAlive(true);
		BasicEnemyArray[13].SetEnemyAlive(true);
		BasicEnemyArray[14].SetEnemyAlive(true);
		BasicEnemyArray[15].SetEnemyAlive(true);
	}
	//doesn't seem to spawn in enemies
	/*if (progFlag == 28){
		//by second door
		BasicEnemyArray[16].SetRobotPos(-18200, 11500, 39000, 'f', 150, 100, 400, "WarriorFrontStanding");
		BasicEnemyArray[17].SetRobotPos(-17800, 11500, 39000, 'f', 150, 100, 400, "WarriorFrontStanding");
		BasicEnemyArray[16].SetEnemyAlive(true);
		BasicEnemyArray[17].SetEnemyAlive(true);
	}*/

	//BOSS ENTITY THINGO
	if (progFlag == 45){
		BasicEnemyArray[20].SetRobotPos(-19850, 10350, 35000, 'f', 150, 200, 100, "ControllerArm");
		BasicEnemyArray[21].SetRobotPos(-21150, 10350, 35000, 'f', 150, 200, 100, "ControllerArm");
		BasicEnemyArray[22].SetRobotPos(-20500, 10500, 35000, 'f', 500, 200, 200, "ControllerBody");
		BasicEnemyArray[23].SetRobotPos(-20500, 10100, 35000, 'f', 500, 200, 200, "ControllerLegs");
		BasicEnemyArray[20].SetEnemyAlive(true);
		BasicEnemyArray[21].SetEnemyAlive(true);
		BasicEnemyArray[22].SetEnemyAlive(true);

	}
	if (tempval){
		for (int i = 0; i < 30; i++){
			BasicEnemyArray[i].CreateRobotObject();
		}
		StartBasicEnemy = false;
	}
	//RobotPosX = 1000, RobotPosY = 10400, RobotPosZ = 40000, RobotWidthX = 150, RobotWidthZ = 100, RobotHeight = 400;
}

void LevelLoad(int level){
	std::string msg = "So it begins..";
	
	drawLoadingScreen(msg, 1);
		obj["trees"] = new ModelLoader("data/3d/trees.obj");
		//CC KIIRA @ http://opengameart.org/content/sky-box-sunny-day
		obj["SKYBOX"] = new ModelLoader("data/3d/sky.obj");
		drawLoadingScreen(msg, 10);
		obj["grass"] = new ModelLoader("data/3d/grass.obj");
		drawLoadingScreen(msg, 20);
		//cc @ http://tf3dm.com/3d-model/wood-barrel-68872.html
		obj["barrel1"] = new ModelLoader("data/3d/barrel1.obj");
		drawLoadingScreen(msg, 30);
		obj["staircase"] = new ModelLoader("data/3d/staircase.obj");
		msg = "Takes a while";
		drawLoadingScreen(msg, 40);
		obj["rock1"] = new ModelLoader("data/3d/rock.obj");
		drawLoadingScreen(msg, 50);
		// CC @
		obj["log"] = new ModelLoader("data/3d/log.obj");
		msg = "its usually faster than this..";
		drawLoadingScreen(msg, 60);
		obj["wall"] = new ModelLoader("data/3d/wall1.obj");
		obj["boss"] = new ModelLoader("data/3d/controllertext.obj");
		drawLoadingScreen(msg, 70);
		//CC @ http://tf3dm.com/3d-model/fallen-log-7859.html
		obj["logfallen"] = new ModelLoader("data/3d/logfallen.obj");
		drawLoadingScreen(msg, 80);
		obj["corridor"] = new ModelLoader("data/3d/corridor.obj");
		obj["table"] = new ModelLoader("data/3d/table.obj");
		// cc @ http://tf3dm.com/3d-model/metal-door-34303.html
		obj["door"] = new ModelLoader("data/3d/door.obj");
		obj["reticle"] = new ModelLoader("data/3d/rec.obj");
		obj["ceilingfan"] = new ModelLoader("data/3d/ceilfan.obj");
		
		msg = "Nearly there";
		drawLoadingScreen(msg, 90);
		es = CEasySound::Instance();
		bgmusic = es->GetSound(es->Load("sounds/Retro_Sci_Fi_Planet.wav"));
		CreateRobots();
		drawLoadingScreen("DONE!", 100);
	
	switch (level)
	{
		case 1:
			viewAngle = -90.0;
			cam.Position(2636.0, 10450.0, 42032.0, -90.0);
			progFlag = -1;
			keys('e', 0, 0);
			break;
		case 2:
			viewAngle = 90.0;
			//cam.Position(1010.50, 10450.0, 40680.4, 90.0);
			progFlag = 10;
			keys('e', 0, 0);
			break;
		case 3:
			viewAngle = -90.0;
			cam.Position(-16350.0, 11500.0, 34000.0, viewAngle);
			progFlag = 33;
			keys('e', 0, 0);
			break;
		default:
...
void LevelDraw(int level){
	...
	case 1:{
		...
		{
			glPushMatrix();
			glTranslatef(2636.0 - 13790 - step, 10000,
				37200.0);
			glRotatef(rot, 0, 1, 0);
		...
		/*glPushMatrix();
		glTranslatef(-577, 100100, 39438.5);
		...
		for (int i = 0; i < 9; i++)
		{
			...
			for (int j = 0; j < 19; j++)
			{
				...
				glPushMatrix();
				glTranslatef(-230 + step2, 10010, 40400+step3);
				glScalef(415, 1, 400);
				obj["grass"]->Draw();
				glPopMatrix();
				step2 -= 825;
		...
		glPushMatrix();
		glTranslatef(-3518.9, 10100, 37037.2);
		glScalef(40, 40, 40);
		obj["barrel1"]->Draw();
		glPopMatrix();

		glPushMatrix();
		glTranslatef(-3307.23, 10100, 40231);
		glRotatef(180, 0, 1, 0);
		glScalef(30, 30, 30);
		obj["barrel1"]->Draw();
		glPopMatrix();
		...
		glPushMatrix();
		glTranslatef(-5430.56, 10000, 38917.8);
		glScalef(130, 130, 130);
		obj["rock1"]->Draw();
		glPopMatrix();

		glPushMatrix();
		glTranslatef(-9431.49, 10000, 40032);
		glScalef(100, 100, 100);
		obj["rock1"]->Draw();
		glPopMatrix();
		
		glPushMatrix();
		glTranslatef(-1025.3, 10000, 34785.5);
		glScalef(150, 150, 150);
		obj["rock1"]->Draw();
		glPopMatrix();

		glPushMatrix();
		glTranslatef(-11121.6, 10000, 35684.2);
		glScalef(150, 150, 150);
		obj["rock1"]->Draw();
		glPopMatrix();
		
		glPushMatrix();
		glTranslatef(-5418.67, 10000, 35165);
		glRotatef(60, 0, 1, 0);
		glScalef(150, 150, 150);
		obj["log"]->Draw();
		glPopMatrix();
		...
		glPushMatrix();
		glTranslatef(-11375.1, 10000, 38932.3);
		glScalef(100, 100, 100);
		obj["logfallen"]->Draw();
		glPopMatrix();
		
		glPushMatrix();
		glTranslatef(-5959.31, 10000, 36714.7);
		glRotatef(90, 0, 1, 0);
		glScalef(150, 150, 150);
		obj["logfallen"]->Draw();
		glPopMatrix();
		...
		glPushMatrix();
		glTranslatef(-14235.1, 10000, 39428.8);
		glRotatef(-30, 0, 1, 0);
		glScalef(150, 150, 150);
		obj["log"]->Draw();
		glPopMatrix();
		...
		{
			glPushMatrix();
				glTranslatef(1490.47 , 11990, 38000.4 - step4);
				glRotatef(ceilrot, 0, 1, 0);
				glScalef(100, 100, 100);
				obj["ceilingfan"]->Draw();
				ceilrot += 1;
				if (ceilrot>360.f)
		...
		{
			glPushMatrix();
			glTranslatef(1874.56 - step5, 11990,  32461.7);
			glRotatef(ceilrot, 0, 1, 0);
			glScalef(100, 100, 100);
			obj["ceilingfan"]->Draw();
			ceilrot += 1;
		...
		glPushMatrix();
		glTranslatef(1490.47, 11050, 38000.4);
		glScalef(100, 100, 100);
		obj["corridor"]->Draw();
		glPopMatrix();

		glPushMatrix();
		glTranslatef(1990.21, 10000, 40652);
		glScalef(45, 41, 50);
		obj["staircase"]->Draw();
		glPopMatrix();

		glPushMatrix();
		glTranslatef(-18065.7, 10000, 40000.7);
		glRotatef(180, 0, 1, 0);
		glScalef(50, 41, 50);
		obj["staircase"]->Draw();
		glPopMatrix();
		...
		glPushMatrix();
		glTranslatef(213.584, 11200, 32589.7);
		glRotatef(90, 0, 1, 0);
		glRotatef(-90, 1, 0, 0);
		glScalef(70, 70, 70);
		obj["table"]->Draw();
		...
		glPushMatrix();
		glTranslatef(-16348.9, 11200, 34536.9);
		//glRotatef(90, 0, 1, 0);
		glRotatef(90, 1, 0, 0);
		glScalef(70, 70, 70);
		obj["table"]->Draw();
		...
		glPushMatrix();
		glTranslatef(-18833.4, 10000, 42960.0);
		...
		glPushMatrix();
		glTranslatef(-20904.7, 11500, 34389.7);
		glScalef(200 + scale , 200 +scale, 200 +scale);
		obj["boss"]->Draw();
		if (grow)
		{
			scale += 1;
		...
		//chnage texture to DECAL (NO lighting) for use with skybox 
		//
		glTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_DECAL);
		glPushMatrix();
		glTranslatef(-245.592, 10000, 40278.1);
...
//--------------------------------------------------------------------------------------
//  ASSIGNMENT1 - Tye's Create functions. Used for making the courtyard area 
//--------------------------------------------------------------------------------------
void CreateWall(const int &XYZ, const int &listNo,
	const GLdouble &xImgSize, const GLdouble &zImgSize,
	const GLdouble &xStart, const GLdouble &yStart, const GLdouble &zStart,
	const GLdouble &xTimes, const GLdouble &zTimes, const int & tempIndex);
void CreateDoorWithEntry(const int &XYZ, const int &listNo,
	const GLdouble &xImgSize, const GLdouble &zImgSize,
	const GLdouble &xStart, const GLdouble &yStart, const GLdouble &zStart,
	const GLdouble &xTimes, const GLdouble &zTimes, const int & tempIndex);

void CreatePost(const int &listNo, const GLdouble &xStart, const GLdouble &yStart, const GLdouble &zStar, const GLdouble &height);
void CreateCortYard();
void CreateBeams(const int &XYZ, const int &listNo, const GLdouble &xStart, const GLdouble &yStart, const GLdouble &zStart, const GLdouble &Times);
void CreateStairCase(const int &XYZ, const int &listNo, const GLdouble &xStart, const GLdouble &yStart, const GLdouble &zStart, const GLdouble &steps);
void CreateWalls() {
	
	//FTR tp.CreateDisplayList(YZ, 122, 128.0, 128.0, 2608.0, 10000.0, 10000.0, 7.0, 243.5);
	//FTR
	//floor
	tp.CreateDisplayList(XZ, 241, 128.0, 64.0, 2680, 10000.0, 43216, -200, -153.38);
	//tp.CreateDisplayList(XZ, 77, 64.0, 64.0, 4848.0, 10000.0, 40816.0, 417.5, 1.0);
	CreateWall(XZ, 1367, 64.0, 64.0, 327, 10001.0, 40800, -248, 1.0, PAVEMENTSIDE_TOP); //side pavenmnt
	CreateWall(XZ, 1367, 64.0, 64.0, 257, 10001.0, 40800, 1, -115.0, PAVEMENTSIDE_LEFT); //side pavenmnt
	CreateWall(XZ, 1367, 64.0, 64.0, 267 - 15872, 10001.0, 40800, 1, -115.0, PAVEMENTSIDE_LEFT); //side pavenmnt

	CreateWall(XZ, 1367, 128.0, 128.0, 2608, 11024.0, 42960.0, -166, -15.0, ABOVE_WINDOW_BLOCK_3); //roof
	//long side
	CreateWall(XY, 1361, 128.0, 128.0, 2608, 10768.0, 42960.0, -167.5, 2.0, WALL_BRICK_XY);		// corner exit to psc block from canteen
	CreateWall(XY, 1367, 128.0, 128.0, 2608, 10000.0, 33400, -167, 8.2, WALL_BRICK_XY);
	...
	for (int i = 0; i < 14; i++)
	{
		...
		for (int j = 0; j < 2; j++)
		{
			...
			CreateWall(XY, 1367, 450, 450, 2608 - 18000 + step, 10100.0 + step2, 33400 + 20, 1, 0.9, WINDOW_LIB_US_B);
			step2 += 500;
		}
		step += 1150;
	}
	//CreateWall(XY, 1367, 450, 450, 2608 - 18000, 10200.0, 33400 + 8, 1, 0.9, WINDOW_LIB_US_B);

	//other side (stairs)
	CreateWall(XY, 1361, 128.0, 128.0, 2608, 10000.0, 41168.0, -10.0, 7.0, WALL_BRICK_XY);
	CreateWall(YZ, 1362, 128.0, 128.0, 1328, 10000.0, 41168.0, 7, -2, WALL_BRICK_YZ);
	CreateWall(XY, 1363, 128.0, 128.0, 2608, 10000.0, 40912.0, -10, 7, WALL_BRICK_XY);
	
	//near stairs entrance
	CreateWall(XY, 1363, 128.0, 128.0, 2608, 10000.0, 39800.0, -7.5, 8, WALL_BRICK_XY);
	CreateWall(YZ, 1363, 128.0, 128.0, 1648, 10768.0, 39800.0, 1.5, -50, WALL_BRICK_YZ);
	CreateWall(XZ, 1367, 128.0, 128.0, 2608, 11024.0, 39800.0, -15, -50, ABOVE_WINDOW_BLOCK_3); //roof
	CreateWall(XZ, 1367, 128.0, 128.0, 688, 11024.0, 39800.0, 7, 10, ABOVE_WINDOW_BLOCK_3); //roof

	//near stairs far side
	CreateWall(XY, 1363, 128.0, 128.0, 2608 -19290 - 560, 10000.0, 39700.0, -12, 8, WALL_BRICK_XY); //rooms
	CreateWall(YZ, 1363, 128.0, 128.0, 2608 - 19290 - 560, 10768.0, 39700.0, 2, -50, WALL_BRICK_YZ); //rooms

	CreateWall(XY, 1363, 128.0, 128.0, 2608 - 19290 - 560, 10000.0, 39800.0 + 1100, -12, 8, WALL_BRICK_XY); //next to stairs
	CreateWall(YZ, 1363, 128.0, 128.0, 2608 - 19290 - 560, 10000.0, 39800.0 + 1100, 8, 2.07, WALL_BRICK_YZ);//next to stairs
	CreateWall(XY, 1363, 128.0, 128.0, 2608 - 19290 - 560, 10000.0, 39800.0 + 1024 + 340, -12, 8, WALL_BRICK_XY);//next to stairs

	CreateWall(YZ, 1363, 128.0, 128.0, 1648 - 19290 - 1100, 10000.0, 41160.0, 8.2, -61, WALL_BRICK_YZ); // back wall
	CreateWall(XZ, 1367, 128.0, 128.0, 2608 - 19290 + 960 -128, 11024.0, 39760, -23, -60, ABOVE_WINDOW_BLOCK_3); //roof
	CreateWall(XZ, 1367, 128.0, 128.0, 2608 - 19290 + 960 - 128, 11024.0, 39760, -14, 10, ABOVE_WINDOW_BLOCK_3); //roof


	//CreateWall(XY, 1364, 128.0, 128.0, -2512, 10000.0, 41168.0, -10.0, 7.0, WALL_BRICK_XY);
	...
	for (int i = 0; i < 15; i++)
	{
		if (i < 13)
		{
			CreatePost(1354, 567.0 + gap, 10000.0, 41040.0, 3.48);
		}
		
		CreateBeams(Y, 1356, 567 + gap, 10950.0, 41168.0, 7);
		gap -= 1378.0;
	}
	gap = 0.0;
	for (int i = 0; i < 6; i++)
	{
		CreatePost(1355, 569.0, 10000.0, 41040.0 + gap, 3.48);
		gap -= 1378.0;
	}
	gap = 0.0;
	for (int i = 0; i < 6; i++)
	{
		CreatePost(1355, -15969, 10000.0, 41040.0 + gap, 3.48);
		gap -= 1378.0;
	}
	//doors
	//tp.CreateYtoZWindowList(1365, 2608, 10000.0, 768.0, 41168.0, 322.0, 1.0, 0.833);
	...
	CreateCortYard();
	CreateBeams(X, 1356, 2600, 10896.0, 41168.0, -72.6);
	CreateBeams(Y, 1356, 567, 10896.0, 41168.0, -30.5);
	CreateBeams(Y, 1356, -16009, 10896.0, 41168.0, -30.5);

	//doors long side
	CreateWall(XY, 1361, 128.0, 128.0, 2608, 10000.0, 42960.0, -12.95, 6, WALL_BRICK_XY);
	CreateDoorWithEntry(XY, 1366, 384.0, 768, 567, 10000.0, 42960, 1, 1, WINDOW_14); 
	glPushMatrix();
	glTranslatef(567, 10000.0, 42960);
	//glRotatef(90, 0, 1, 0);
	glScalef(50, 50, 50);
	obj["door"]->Draw();
	glPopMatrix();

	CreateWall(XY, 1361, 128.0, 128.0, 567, 10000.0, 42960.0, -7.77, 6, WALL_BRICK_XY);
	CreateDoorWithEntry(XY, 1366, 384.0, 768, -811, 10000.0, 42960, 1, 1, WINDOW_14); 
	glPushMatrix();
	glTranslatef(-811, 10000.0, 42960);
	//glRotatef(90, 0, 1, 0);
	glScalef(50, 50, 50);
	obj["door"]->Draw();
	glPopMatrix();
	CreateWall(XY, 1361, 128.0, 128.0, -811, 10000.0, 42960.0, -2.47, 6, WALL_BRICK_XY);
	CreateDoorWithEntry(XY, 1366, 384.0, 768, -1511, 10000.0, 42960, 1, 1, WINDOW_14); 
	glPushMatrix();
	glTranslatef(-1511, 10000.0, 42960);
	//glRotatef(90, 0, 1, 0);
	glScalef(50, 50, 50);
	obj["door"]->Draw();
	glPopMatrix();

	CreateWall(XY, 1361, 128.0, 128.0, -1511, 10000.0, 42960.0, -34.6, 6, WALL_BRICK_XY);
	CreateDoorWithEntry(XY, 1366, 384.0, 768, -6323, 10000.0, 42960, 1, 1, WINDOW_14); 
	
	glPushMatrix();
	glTranslatef(-6323, 10000.0, 42960);
	//glRotatef(90, 0, 1, 0);
	glScalef(50, 50, 50);
	obj["door"]->Draw();
	glPopMatrix();
	CreateWall(XY, 1361, 128.0, 128.0, -6323, 10000.0, 42960.0, -2.47, 6, WALL_BRICK_XY);
	CreateDoorWithEntry(XY, 1366, 384.0, 768, -7023, 10000.0, 42960, 1, 1, WINDOW_14);
	glPushMatrix();
	glTranslatef(-7023, 10000.0, 42960);
	//glRotatef(90, 0, 1, 0);
	glScalef(50, 50, 50);
	obj["door"]->Draw();
	glPopMatrix();
	CreateWall(XY, 1361, 128.0, 128.0, -7023, 10000.0, 42960.0, -18.21, 6, WALL_BRICK_XY);
	CreateDoorWithEntry(XY, 1366, 384.0, 768, -9737, 10000.0, 42960, 1, 1, WINDOW_14);
	glPushMatrix();
	glTranslatef(-9737, 10000.0, 42960);
	//glRotatef(90, 0, 1, 0);
	glScalef(50, 50, 50);
	obj["door"]->Draw();
	glPopMatrix();

	CreateWall(XY, 1361, 128.0, 128.0, -9737, 10000.0, 42960.0, -15.75, 6, WALL_BRICK_XY);
	CreateDoorWithEntry(XY, 1366, 384.0, 768, -12137, 10000.0, 42960, 1, 1, WINDOW_14);
	glPushMatrix();
	glTranslatef(-12137, 10000.0, 42960);
	//glRotatef(90, 0, 1, 0);
	glScalef(50, 50, 50);
	obj["door"]->Draw();
	glPopMatrix();
	CreateWall(XY, 1361, 128.0, 128.0, -12137, 10000.0, 42960.0, -17.81, 6, WALL_BRICK_XY);
	CreateDoorWithEntry(XY, 1366, 384.0, 768, -14800, 10000.0, 42960, 1, 1, WINDOW_14);
	glPushMatrix();
	glTranslatef(-14800, 10000.0, 42960);
	//glRotatef(90, 0, 1, 0);
	glScalef(50, 50, 50);
	obj["door"]->Draw();
	glPopMatrix();
	CreateWall(XY, 1361, 128.0, 128.0, -14800, 10000.0, 42960.0, -9.72, 6, WALL_BRICK_XY);
	CreateDoorWithEntry(XY, 1366, 384.0, 768, -16427, 10000.0, 42960, 1, 1, WINDOW_14);
	glPushMatrix();
	glTranslatef(-16427, 10000.0, 42960);
	//glRotatef(90, 0, 1, 0);
	glScalef(50, 50, 50);
	obj["door"]->Draw();
	glPopMatrix();
	CreateWall(XY, 1361, 128.0, 128.0, -16427, 10000.0, 42960.0, -19, 6, WALL_BRICK_XY);


	//doors entrnace side
	
	CreateWall(YZ, 1361, 128.0, 128.0, 1648, 10000.0, 39800, 6, -0.91, WALL_BRICK_YZ);
	CreateDoorWithEntry(YZ, 1366, 768, 384, 1648, 10000.0, 39300.0, 1, 1, WINDOW_14);
	glPushMatrix();
	glTranslatef(1648, 10000.0, 39300.0);
	glTranslatef(-70, 0, 390);
	glRotatef(90, 0, 1, 0);
	glScalef(53, 50, 50);
	obj["door"]->Draw();
	glPopMatrix();
	CreateWall(YZ, 1361, 128.0, 128.0, 1648, 10000.0, 39300.0, 6, -10.35, WALL_BRICK_YZ);
	CreateDoorWithEntry(YZ, 1366, 768, 384, 1648, 10000.0, 37588, 1, 1, WINDOW_14);
	glPushMatrix();
	glTranslatef(1648, 10000.0, 37588);
	glTranslatef(-70, 0, 390);
	glRotatef(90, 0, 1, 0);
	glScalef(53, 50, 50);
	obj["door"]->Draw();
	glPopMatrix();
	CreateWall(YZ, 1361, 128.0, 128.0, 1648, 10000.0, 37588.0, 6, -11.72, WALL_BRICK_YZ);
	CreateDoorWithEntry(YZ, 1366, 768, 384, 1648, 10000.0, 35704, 1, 1, WINDOW_14);
	CreateWall(YZ, 1361, 128.0, 128.0, 1648, 10000.0, 35705, 6, -18, WALL_BRICK_YZ);
	glPushMatrix();
	glTranslatef(1648, 10000.0, 35704);
	glTranslatef(-70, 0, 390);
	glRotatef(90, 0, 1, 0);
	glScalef(53, 50, 50);
	obj["door"]->Draw();
	glPopMatrix();
	//doors far side
	CreateWall(YZ, 1363, 128.0, 128.0, -17242, 10000.0, 39700, 6, -6.25, WALL_BRICK_YZ);
	CreateDoorWithEntry(YZ_FLIP, 1366, 768, 384, -17242, 10000.0, 38900 - 384, 1, 1, WINDOW_14);

	glPushMatrix();
	glTranslatef(-17242, 10000.0, 38900);
	glTranslatef(-280, 0, 5);
	glRotatef(90, 0, 1, 0);
	glScalef(53, 50, 50);
	obj["door"]->Draw();
	glPopMatrix();
	CreateWall(YZ, 1363, 128.0, 128.0, -17242, 10000.0, 38900 - 384, 6, -9.5, WALL_BRICK_YZ);
	CreateDoorWithEntry(YZ_FLIP, 1366, 768, 384, -17242, 10000.0, 37300 - 384, 1, 1, WINDOW_14);
	glPushMatrix();
	glTranslatef(-17242, 10000.0, 37300);
	glTranslatef(-280, 0, 5);
	glRotatef(90, 0, 1, 0);
	glScalef(53, 50, 50);
	obj["door"]->Draw();
	glPopMatrix();
	CreateWall(YZ, 1363, 128.0, 128.0, -17242, 10000.0, 37300 - 384, 6, -11.07, WALL_BRICK_YZ);
	CreateDoorWithEntry(YZ_FLIP, 1366, 768, 384, -17242, 10000.0, 35500 - 384, 1, 1, WINDOW_14);
	glPushMatrix();
	glTranslatef(-17242, 10000.0, 35500);
	glTranslatef(-280, 0, 5);
	glRotatef(90, 0, 1, 0);
	glScalef(53, 50, 50);
	obj["door"]->Draw();
	glPopMatrix();
	CreateWall(YZ, 1363, 128.0, 128.0, -17242, 10000.0, 35500 - 384, 6, -6.38, WALL_BRICK_YZ);
	CreateDoorWithEntry(YZ_FLIP, 1366, 768, 384, -17242, 10000.0, 34300 - 384, 1, 1, WINDOW_14);
	glPushMatrix();
	glTranslatef(-17242, 10000.0, 34300);
	glTranslatef(-280, 0, 5);
	glRotatef(90, 0, 1, 0);
	glScalef(53, 50, 50);
	obj["door"]->Draw();
	glPopMatrix();
	CreateWall(YZ, 1363, 128.0, 128.0, -17242, 10000.0, 34300 - 384, 6, -4, WALL_BRICK_YZ);

	////entrance side stairs
	//CreateStairCase(YZ, 1359, 1328 + 32*10, 10000.0, 40912.0 - 512, 10);
	//CreateWall(XZ, 1359, 128.0, 128.0, 2608, 10000.0 + 420 + 22, 40912.0, -2.5, -8.5, STEP_PAVING_1);
	//CreateStairCase(XZ, 1360, 2608 -32*12, 10000.0 + 420 - 42, 40912 - 128 *8.5, 10);

	////far sidee stairs
	//CreateStairCase(XZ, 1359, 1648 - 19290 - 42, 10000.0 - 42, 40912.0 - 512, 10);
	//CreateWall(XZ, 1359, 128.0, 128.0, 1648 - 19290 - 590, 10000.0 + 420 + 22, 40912.0, -3.5, -8.5, STEP_PAVING_1);
	//CreateStairCase(YZ, 1360, 1648 - 19290 - 590, 10000.0 + 420, 40912 - 128 * 8.5, 10);

}
void CreatePost(const int &listNo, const GLdouble &xStart, const GLdouble &yStart, const GLdouble &zStart, const GLdouble &height) {
	//add thinkness option
	tp.CreateDisplayList(XY, listNo, 128.0, 256.0, xStart, yStart, zStart, 1.0, height);
	...
	glCallList(listNo);
	glPopMatrix();
	tp.CreateDisplayList(YZ, listNo, 256.0, 128.0, xStart, yStart, zStart, height, 1.0);
...
void CreateDoorWithEntry(const int &XYZ, const int &listNo,
	...
	const GLdouble &xStart, const GLdouble &yStart, const GLdouble &zStart,
	const GLdouble &xTimes, const GLdouble &zTimes, const int & tempIndex) {
	//CreateWall(XYZ, listNo, xImgSize, zImgSize, xStart, yStart, zStart, xTimes, zTimes,  )
	//create display lits xImgSize, zImgSize, xStart, yStart, zStart, xTimes, zTimes
	switch (XYZ)
	{
	case XY:{
		CreateWall(YZ, listNo, 128.0, 128.0, xStart, yStart, zStart, 6, 2, WALL_BRICK_YZ);
		CreateWall(YZ, listNo, 128.0, 128.0, xStart+xImgSize, yStart, zStart, 6, 2, WALL_BRICK_YZ);
		CreateWall(XZ, listNo, 128, 128.0, xStart, yStart + zImgSize, zStart, 3, 2, WALL_BRICK_YZ);
		CreateWall(XZ, listNo, 32, 32.0, xStart + 32, yStart + 2, zStart, 10, 7, CARPET);
		tp.CreateDisplayList(XYZ, listNo, xImgSize, zImgSize, xStart, yStart, zStart, xTimes, zTimes);
		//draw (glcalllist)
		glBindTexture(GL_TEXTURE_2D, tp.GetTexture(tempIndex));
		glPushMatrix();
			glTranslatef(0.0, 0.0, 256);
			glCallList(listNo);
		glPopMatrix();
	}
		break;
	case YZ:{
		CreateWall(XY, listNo, 128.0, 128.0, xStart, yStart, zStart, 1, 6, WALL_BRICK_XY);
		CreateWall(XY, listNo, 128.0, 128.0, xStart, yStart, zStart + zImgSize, 1, 6, WALL_BRICK_XY);
		CreateWall(XZ, listNo, 128, 128.0, xStart, yStart + xImgSize, zStart, 1, 3, WALL_BRICK_YZ);
		CreateWall(XZ, listNo, 32, 32.0, xStart, yStart + 2, zStart + 32, 4, 10, CARPET);
		tp.CreateDisplayList(XYZ, listNo, xImgSize, zImgSize, xStart, yStart, zStart, xTimes, zTimes);
		//draw (glcalllist)
		glBindTexture(GL_TEXTURE_2D, tp.GetTexture(tempIndex));
		glPushMatrix();
	...
	//binding box tp.CreateDisplayList(YZ, 122, 128.0, 128.0, 2608.0, 10000.0, 10000.0, 7.0, 243.5);
	/*cam.SetAABBMaxX(20, 2608.0);
	cam.SetAABBMinX(20, 0.0);
	cam.SetAABBMaxZ(20, 41236.0);
...
void CreateCortYard() {
	...
	/*tp.CreateDisplayList(XZ, 1355, 128.0, 64.0, -2512, 10000.0, 41168.0, 30, -33.0);
	glCallList(1355);*/
}
void CreateBeams(const int &XYZ, const int &listNo, const GLdouble &xStart, const GLdouble &yStart, const GLdouble &zStart, const GLdouble &Times) {
	//add thinkness option
	...
	{
	case X:{
		tp.CreateDisplayList(XY, listNo, 256.0, 256.0, xStart, yStart, zStart, Times, 0.75);
		...
		tp.CreateDisplayList(XZ, listNo, 256.0, 128.0, xStart, yStart, zStart - 128, Times, 1);  // above west exit
		glBindTexture(GL_TEXTURE_2D, tp.GetTexture(ABOVE_UNDER_POSTS_2));
		glCallList(listNo);
	}
		   break;
	case Y: {
		tp.CreateDisplayList(YZ, listNo, 256.0, 256.0, xStart, yStart, zStart, 0.75, Times);
		...
		tp.CreateDisplayList(XZ, listNo, 128.0, 256.0, xStart, yStart, zStart, 1, Times);  // above west exit
...
void CreateStairCase(const int &XYZ, const int &listNo, const GLdouble &xStart, const GLdouble &yStart, const GLdouble &zStart, const GLdouble &steps){
	...
	case YZ:{
		...
		for (int i = 0; i < steps; i++)
		{
			...
			tp.CreateDisplayList(YZ, listNo, 42.0, 512.0, xStart, yStart, zStart, 1.0, 0.942);
			glPushMatrix();
				glTranslatef(step, -step2 - 42, 0);
				glCallList(listNo);
			glPopMatrix();

			tp.CreateDisplayList(XZ, listNo, 64.0, 512.0, xStart, yStart, zStart, 1.0, 0.942);
			glPushMatrix();
	...
	case XZ:{
		...
		{
			glBindTexture(GL_TEXTURE_2D, tp.GetTexture(STEPS_LIBRARY));
			tp.CreateDisplayList(XZ, listNo, 64, 512.0, xStart, yStart, zStart, 1.0, 0.942);
			glPushMatrix();
				glTranslatef(-step, -step2, 0.0);
				glCallList(listNo);
			glPopMatrix();
			glBindTexture(GL_TEXTURE_2D, tp.GetTexture(STEPS_LIBRARY));
			tp.CreateDisplayList(YZ, listNo, 42, 512.0, xStart, yStart, zStart, 1.0, 0.942);
			glPushMatrix();
				glTranslatef(-step, -step2, 0);
				glCallList(listNo);
			glPopMatrix();
			step += 64.0;
			step2 += -42.0;
...
int main(int argc, char **argv)
{
	...
	glutCreateWindow("LaseWorld Client | ICT290 Assignment 2");
	...
	glutPassiveMotionFunc(RolloverMouse);
...
void loadHUD(){
	...
	textures["ScoreBar"] = SOIL_load_OGL_texture
		(
		"data/HUD/ScoreBar.png",
		SOIL_LOAD_AUTO,
		SOIL_CREATE_NEW_ID,
		SOIL_FLAG_MIPMAPS | SOIL_FLAG_INVERT_Y | SOIL_FLAG_NTSC_SAFE_RGB | SOIL_FLAG_COMPRESS_TO_DXT
		);

	textures["ArmourBar"] = SOIL_load_OGL_texture
		(
		"data/HUD/ArmourBar.png",
		SOIL_LOAD_AUTO,
		SOIL_CREATE_NEW_ID,
		SOIL_FLAG_MIPMAPS | SOIL_FLAG_INVERT_Y | SOIL_FLAG_NTSC_SAFE_RGB | SOIL_FLAG_COMPRESS_TO_DXT
		);

	textures["AmmoBar"] = SOIL_load_OGL_texture
		(
		"data/HUD/AmmoBar.png",
		SOIL_LOAD_AUTO,
		SOIL_CREATE_NEW_ID,
		SOIL_FLAG_MIPMAPS | SOIL_FLAG_INVERT_Y | SOIL_FLAG_NTSC_SAFE_RGB | SOIL_FLAG_COMPRESS_TO_DXT
		);
	...
	textures["MainMenu"] = SOIL_load_OGL_texture
		(
		"data/HUD/MainMenu.png",
		SOIL_LOAD_AUTO,
		SOIL_CREATE_NEW_ID,
		SOIL_FLAG_MIPMAPS | SOIL_FLAG_INVERT_Y | SOIL_FLAG_NTSC_SAFE_RGB | SOIL_FLAG_COMPRESS_TO_DXT
		);

	textures["StageMenu"] = SOIL_load_OGL_texture
		(
		"data/HUD/StageMenu.png",
		SOIL_LOAD_AUTO,
		SOIL_CREATE_NEW_ID,
		SOIL_FLAG_MIPMAPS | SOIL_FLAG_INVERT_Y | SOIL_FLAG_NTSC_SAFE_RGB | SOIL_FLAG_COMPRESS_TO_DXT
		);

	textures["ScoreMenu"] = SOIL_load_OGL_texture
		(
		"data/HUD/ScoreMenu.png",
		SOIL_LOAD_AUTO,
		SOIL_CREATE_NEW_ID,
		SOIL_FLAG_MIPMAPS | SOIL_FLAG_INVERT_Y | SOIL_FLAG_NTSC_SAFE_RGB | SOIL_FLAG_COMPRESS_TO_DXT
		);

	textures["ControlsMenu"] = SOIL_load_OGL_texture
		(
		"data/HUD/ControlsMenu.png",
		SOIL_LOAD_AUTO,
		SOIL_CREATE_NEW_ID,
		SOIL_FLAG_MIPMAPS | SOIL_FLAG_INVERT_Y | SOIL_FLAG_NTSC_SAFE_RGB | SOIL_FLAG_COMPRESS_TO_DXT
		);

	textures["stage1Rollover"] = SOIL_load_OGL_texture
		(
		"data/HoverOver/Stage1.png",
		SOIL_LOAD_AUTO,
		SOIL_CREATE_NEW_ID,
		SOIL_FLAG_MIPMAPS | SOIL_FLAG_INVERT_Y | SOIL_FLAG_NTSC_SAFE_RGB | SOIL_FLAG_COMPRESS_TO_DXT
		);

	textures["stage2Rollover"] = SOIL_load_OGL_texture
		(
		"data/HoverOver/Stage2.png",
		SOIL_LOAD_AUTO,
		SOIL_CREATE_NEW_ID,
		SOIL_FLAG_MIPMAPS | SOIL_FLAG_INVERT_Y | SOIL_FLAG_NTSC_SAFE_RGB | SOIL_FLAG_COMPRESS_TO_DXT
		);

	textures["stage3Rollover"] = SOIL_load_OGL_texture
		(
		"data/HoverOver/Stage3.png",
		SOIL_LOAD_AUTO,
		SOIL_CREATE_NEW_ID,
		SOIL_FLAG_MIPMAPS | SOIL_FLAG_INVERT_Y | SOIL_FLAG_NTSC_SAFE_RGB | SOIL_FLAG_COMPRESS_TO_DXT
		);

	textures["controlRollover"] = SOIL_load_OGL_texture
		(
		"data/HoverOver/controlRollover.png",
		SOIL_LOAD_AUTO,
		SOIL_CREATE_NEW_ID,
		SOIL_FLAG_MIPMAPS | SOIL_FLAG_INVERT_Y | SOIL_FLAG_NTSC_SAFE_RGB | SOIL_FLAG_COMPRESS_TO_DXT
		);

	textures["stageRollover"] = SOIL_load_OGL_texture
		(
		"data/HoverOver/stageRollover.png",
		SOIL_LOAD_AUTO,
		SOIL_CREATE_NEW_ID,
		SOIL_FLAG_MIPMAPS | SOIL_FLAG_INVERT_Y | SOIL_FLAG_NTSC_SAFE_RGB | SOIL_FLAG_COMPRESS_TO_DXT
		);

	textures["playRollover"] = SOIL_load_OGL_texture
		(
		"data/HoverOver/playRollover.png",
		SOIL_LOAD_AUTO,
		SOIL_CREATE_NEW_ID,
		SOIL_FLAG_MIPMAPS | SOIL_FLAG_INVERT_Y | SOIL_FLAG_NTSC_SAFE_RGB | SOIL_FLAG_COMPRESS_TO_DXT
		);

	textures["scoreRollover"] = SOIL_load_OGL_texture
		(
		"data/HoverOver/ScoreRollover.png",
		SOIL_LOAD_AUTO,
		SOIL_CREATE_NEW_ID,
		SOIL_FLAG_MIPMAPS | SOIL_FLAG_INVERT_Y | SOIL_FLAG_NTSC_SAFE_RGB | SOIL_FLAG_COMPRESS_TO_DXT
		);

	textures["exitRollover"] = SOIL_load_OGL_texture
		(
		"data/HoverOver/ExitRollover.png",
		SOIL_LOAD_AUTO,
		SOIL_CREATE_NEW_ID,
		SOIL_FLAG_MIPMAPS | SOIL_FLAG_INVERT_Y | SOIL_FLAG_NTSC_SAFE_RGB | SOIL_FLAG_COMPRESS_TO_DXT
		);

	textures["MainMenuRollover"] = SOIL_load_OGL_texture
		(
		"data/HoverOver/mainMenuRollover.png",
		SOIL_LOAD_AUTO,
		SOIL_CREATE_NEW_ID,
		SOIL_FLAG_MIPMAPS | SOIL_FLAG_INVERT_Y | SOIL_FLAG_NTSC_SAFE_RGB | SOIL_FLAG_COMPRESS_TO_DXT
		);
...
void myinit()
{
	...
	//TODO: http://stackoverflow.com/questions/5206373/opengl-texture-transparency-doesnt-work
	glEnable(GL_BLEND);
	...
	/*cam.Position(32720.0, 9536.0,
				 4800.0, 180.0);*/
	//Looking toward courtyard area
	cam.Position(-16487.2, 10450, 39317.4, -90.0);
	//cam.Position(2636.0 - 17290, 11450.0, 42032.0, -90.0);
	//cam.Position(0,0,0, -90.0); 
...
int Ammo = 12;
//if you change the score max you have to change teh array max currently set to 6 figures
void DrawTextureSquare(std::string FileName, GLfloat xmin, GLfloat xmax, GLfloat ymin, GLfloat ymax){
...
void DrawNumbers(int number, int NumberStringPos, GLfloat xmin, GLfloat xmax, GLfloat ymin, GLfloat ymax, GLfloat GapSize){
	for (int i = 0; i <= 10; i++){
		if (number == i){
			...
			glBegin(GL_QUADS);
			glTexCoord2f(0, 0);
			glVertex2f(xmin + (NumberStringPos * GapSize), ymin);
			glTexCoord2f(1, 0);
			glVertex2f(xmax + (NumberStringPos*GapSize), ymin);
			glTexCoord2f(1, 1);
			glVertex2f(xmax + (NumberStringPos * GapSize), ymax);
			glTexCoord2f(0, 1);
			glVertex2f(xmin + (NumberStringPos * GapSize), ymax);
			glEnd();
...
void DisplayHUD(){
	...
	//for loop to put in numbers for score
	const int ScoreSize = 6;
	int scoreArray[ScoreSize];
	int tempScore = score;

	//draws the bars for ammo, score and armour
	// TEST LINE score = ymouse;
	DrawTextureSquare("AmmoBar", 0, 100, 0, 50);
	DrawTextureSquare("ScoreBar", 700, 800, 450, 500);
	DrawTextureSquare("ArmourBar", 700, 800, 0, 50);
	for (int i = ScoreSize - 1; i >= 0; i--) {
		scoreArray[i] = tempScore % 10;
		tempScore /= 10;
		DrawNumbers(scoreArray[i], i, 740, 745, 465, 485, 10);
	}

	//for loop to put numbers for armour;
	int ArmourArray[3];
	int tempArmour = Armour;
	for (int i = 2; i >= 0; i--) {
		ArmourArray[i] = tempArmour % 10;
		tempArmour /= 10;
		DrawNumbers(ArmourArray[i], i, 760, 765, 15, 35, 10);
	}
	...
	//for loop for numbers in Ammo
	int AmmoArray[2];
	int tempAmmo = Ammo;
	for (int i = 1; i >= 0; i--) {
		AmmoArray[i] = tempAmmo % 10;
		tempAmmo /= 10;
		DrawNumbers(AmmoArray[i], i, 60, 75, 15, 35, 20);
	}
	glMatrixMode(GL_PROJECTION);
	glPopMatrix();
	glMatrixMode(GL_MODELVIEW);
...
void DisplayMenu(){
	...
	if (MainMenu){
		DrawTextureSquare("MainMenu", 200, 600, 0, 500);
	}
	if (ControlMenu){
		DrawTextureSquare("ControlsMenu", 200, 600, 0, 500);
	}
	if (StageMenu){
		DrawTextureSquare("StageMenu", 200, 600, 0, 500);
	}
	if (ScoreBoard){
		DrawTextureSquare("ScoreMenu", 200, 600, 0, 500);
	}

	//below are rollover buttons - NEED IMAGES FOR THEM
	if (stage1Rollover){
		DrawTextureSquare("stage1Rollover", 238.4, 561.6, 500 - 173.32, 500 - 108);
	}
	if (stage2Rollover){
		DrawTextureSquare("stage2Rollover", 238.4, 561.6, 500 - 275, 500 - 209.62);
	}
	if (stage3Rollover){
		DrawTextureSquare("stage3Rollover", 238.4, 561.6, 500 - 376.7, 500 - 311.26);
	}
	if (controlRollover){
		DrawTextureSquare("controlRollover", 238.4, 561.6, 500 - 383, 500 - 329);
	}
	if (stageRollover){
		DrawTextureSquare("stageRollover", 238.4, 561.6, 500 - 215, 500 - 161);
	}
	if (playRollover){
		DrawTextureSquare("playRollover", 238.4, 561.6, 500 - 131, 500 - 77);
	}
	if (scoreRollover){
		DrawTextureSquare("scoreRollover", 238.4, 561.6, 500 - 299, 500 - 245);
	}
	if (exitRollover){
		DrawTextureSquare("exitRollover", 238.4, 561.6, 500 - 467, 500 - 413);
	}
	if (scoreMainMenuRollover){
		DrawTextureSquare("MainMenuRollover", 238.4, 561.6, 500 - 467, 500 - 413);
	}
	if (controlMainMenuRollover){
		DrawTextureSquare("MainMenuRollover", 238.4, 561.6, 500 - 467, 500 - 413);
	}
	if (stageMainMenuRollover){
		DrawTextureSquare("MainMenuRollover", 238.4, 561.6, 500 - 467, 500 - 413);
	}
...
//remove later Create robots should do everything this does
...
void Display()
{
	...
	if (play == false){
		...
		if (currentLevel == 1)
		{
			...
			//BasicEnemy.CreateRobotObject();
			//BEN call for create robots
			
			BasicEnemyArray[4].SetRobotPos(-4000 + loop, 11400, 34000 + loop, 'f', 150, 200, 100, "TestTargetObserver");
			BasicEnemyArray[1].SetRobotPos(500, 10400, 40000 + loop, 'f', 150, 100, 400, "TestTargetWarrior");
			BasicEnemyArray[2].SetRobotPos(-2000, 10400, 35000 + loop, 'f', 150, 100, 400, "TestTargetWarrior");
			BasicEnemyArray[3].SetRobotPos(-3000, 10400, 38000+loop, 'f', 150, 100, 400, "TestTargetWarrior");
	...
	/*glPushMatrix();
		glColor3f(0, 1, 0);
		glTranslatef(2636.0 - 17090, 10100.0,
			42032.0);
		glScalef(30, 30, 30);
		obj[1]->Draw();
	glPopMatrix();*/
...
/***********************************************************************************************************
* @brief On-Rails Movement and Cover function
*
* This function is the primary function for gameplay, governing the on-rails movement of the player camera
* as well as including the function for taking cover in combat
*
* @version 0.9a
* @author Brandon Lim (32672236)
* @date 26/10/2016
* @todo Final Navigation down to lower level and into bossfight arena; call enemy functions when enemy functions are done
* @note Uses global variables inCover, inCombat, viewAngle, activeFlag and progFlag. Also uses global constant TIMERTICK.
* @note Calls itself using glutTimerFunc for regular interval. Also affects default input functions used by Shay.
***********************************************************************************************************/
void onRails(int val){
	if (activeFlag != 0){
		//cover stuff here.
		if (inCombat){
			if (inCover){
				if (progFlag == 3 || progFlag == 46){ //base height = 10450
					if (cam.GetUD() >= 10300.0){
						cam.Position(cam.GetLR(), cam.GetUD() - 75.0, cam.GetFB(), viewAngle);
					}
				}
				else{
					if (progFlag == 19 || progFlag == 24 || progFlag == 29){
						if (cam.GetUD() >= 11200){
							cam.Position(cam.GetLR(), cam.GetUD() - 75.0, cam.GetFB(), viewAngle);
						}
					}
				...
				if (progFlag == 3 || progFlag == 46){ //lower level cover
					if (cam.GetUD() < 10450.0){
						cam.Position(cam.GetLR(), cam.GetUD() + 75.0, cam.GetFB(), viewAngle);
					}
				}
				else{ //upper level cover
					if (progFlag == 19 || progFlag == 24 || progFlag == 29){
						if (cam.GetUD() < 11500){
							cam.Position(cam.GetLR(), cam.GetUD() + 75.0, cam.GetFB(), viewAngle);
						}
		...
		case -1:
			...
			break;
		case 0:
			cam.Position(cam.GetLR() - 100, cam.GetUD(), cam.GetFB(), viewAngle);
			//cam.Position(2636.0, 10450.0, 42032.0, -90.0);
			if (cam.GetLR() < -3100.00){
				progFlag++;
			}
			break;
		case 1:
			cam.Position(cam.GetLR(), cam.GetUD(), cam.GetFB(), viewAngle);
			if (viewAngle != 0){
				viewAngle += 5;
			}
			else {
			...
			break;
		case 2:
			cam.Position(cam.GetLR(), cam.GetUD(), cam.GetFB() - 100, viewAngle);
			if (cam.GetFB() <= 40900){
				cam.DirectionFB(0);
				cam.Position(-3180.0, 10450.0, 40850.0, 0.0);
				progFlag++;
			}
			CreateRobots();
			break;
		case 3:
			...
			//enemy spawn cases
			//wait for now...
			glutTimerFunc(TIMERTICKFIRE, FireRobot, 1);
			if (score >= 2000){
				progFlag++;
			}
			break;
		...
		case 11:
			...
			break;
		case 12:
			cam.Position(cam.GetLR() + 15, cam.GetUD() + 8, cam.GetFB(), 90.0);
			if (cam.GetLR() >= 2400.0){
				viewAngle = 90.0;
				progFlag++;
			}
		...
		case 13:
			...
			break;
		case 14:
			cam.Position(cam.GetLR(), cam.GetUD(), cam.GetFB() - 25, viewAngle);
			if (cam.GetFB() < 40000){
				progFlag++;
			}
			break;
		...
		case 19:
			if (inCombat == 0){
				cam.Position(1430.0, 11500.0, 39500.0, viewAngle);
			}
			// First Encounter
			inCombat = 1;
			glutTimerFunc(TIMERTICKFIRE, FireRobot, 1);
		...
		case 24:
			if (inCombat == 0){
				cam.Position(405.0, 11500.0, 32570.0, viewAngle);
			}
			//Second Encouter
			inCombat = 1;
			glutTimerFunc(TIMERTICKFIRE, FireRobot, 1);
		...
		case 25:
			...
			cam.Position(cam.GetLR(), cam.GetUD(), cam.GetFB() + 25, viewAngle);
			if (cam.GetFB() > 33200.0){
				progFlag++;
			}
			break;
		case 26:
			cam.Position(cam.GetLR() - 100, cam.GetUD(), cam.GetFB(), viewAngle);
			if (cam.GetLR() < -16300.0){
				cam.DirectionFB(0);
				progFlag++;
			}
			break;
		...
		case 30:
			...
			break;
		case 31:
			cam.Position(cam.GetLR(), cam.GetUD(), cam.GetFB() + 30, viewAngle);
			if (cam.GetFB() > 40500.0){
				cam.DirectionFB(0);
				progFlag++;
			}
			break;
		case 32:
			...
			if (viewAngle != -90.0){
				...
				progFlag++;
				cam.Position(-17120.0, 11500.0, 40500.0, -90.0);
			}
			break;
			//*****Stage 3*****
		case 33:
			cam.Position(cam.GetLR() - 100, cam.GetUD(), cam.GetFB(), viewAngle);
			if (cam.GetLR() < -17300.0){
				progFlag++;
			}
			break;
		case 34:
			cam.Position(cam.GetLR() - 15, cam.GetUD() - 9, cam.GetFB(), -90.0);
			if (cam.GetLR() <= -18400.0){
				viewAngle = -90.0;
				progFlag++;
			}
			break;
		...
		case 46:
			//BOSS FIGHT!
			if (inCombat == 0){
				cam.Position(-20110.0, 10450.0, 41300.0, 0.0);
			}
			inCombat = 1;
			glutTimerFunc(TIMERTICKFIRE, FireRobot, 1);
			break;
		...
		glutPostRedisplay();
		glutTimerFunc(TIMERTICK, onRails, val);
	}
	else{
		cam.DirectionLR(0);
		cam.DirectionFB(0);
...
void keys2(unsigned char key, int x, int y){
	...
	case 'm':
		...
		break;
		//Brandon's Start Funcs:
	case 'e':
	case 'E':
		if (progFlag==-1){
			viewAngle = -90.0;
			cam.Position(2636.0, 10450.0, 42032.0, viewAngle);
			//progFlag = 0;
		}
		score = 0;
		activeFlag = 1;
		glutTimerFunc(TIMERTICK, onRails, 0);
		break;
...
void checkHit(int x, int y){
	...
	GLdouble nearx = 0, neary = 0, nearz = 0;
	GLdouble farx = 0, fary = 0, farz = 0;
	...
	gluUnProject((GLdouble)x, (GLdouble)realy, 0.0, mvmatrix,
		projmatrix, viewport, &nearx, &neary, &nearz);
	gluUnProject((GLdouble)x, (GLdouble)realy, 1.0, mvmatrix,
		projmatrix, viewport, &farx, &fary, &farz);

	GLdouble vecx = farx - nearx, vecy = fary - neary, vecz = farz - nearz;
	GLdouble vecl = sqrt((vecx*vecx) + (vecy*vecy) + (vecz*vecz));
	vecx = vecx / vecl;
	vecy = vecy / vecl;
	vecz = vecz / vecl;

	for (int i = 0; i < 30; i++){
		GLuint tmpx = BasicEnemyArray[i].getBotX();
		GLuint tmpy = BasicEnemyArray[i].getBotY();
		GLuint tmpz = BasicEnemyArray[i].getBotZ();

		for (int j = 0; j < 10000; j++){
			GLuint tmx = cam.GetLR()+vecx*j, tmy = cam.GetUD()+vecy*j, tmz = cam.GetFB()+vecz*j;
			if (tmx >= tmpx - 150 && tmx <= tmpx + 150){
				if (tmz >= tmpz - 100 && tmz <= tmpz + 100){
					if (tmy >= tmpy - 400 && tmy <= tmpy + 400){
						score += 100;
						BasicEnemyArray[i].EnemyHP = BasicEnemyArray[i].EnemyHP - 1;
						BasicEnemyArray[i].SetRobotPos(0, 0, 0, 'r', 0, 0, 0, "WarriorFrontHurt");
...
//--------------------------------------------------------------------------------------
//  Mouse Buttons
// Used By Ben Porter for menu buttons
//--------------------------------------------------------------------------------------
void Mouse(int button, int state, int x, int y)
{
	double mouseX = ((double)x / (double)width);
	double mouseY = ((double)y / (double)height);
	...
	if ((button == GLUT_LEFT_BUTTON) && (state == GLUT_DOWN))
	{
		...
		//updates ammo is mouse is clicked while in the game //[BRANDON] Added hit check call.
		if ((Ammo > 0) && (play) && (inCombat)&&(!inCover)){
			Ammo -= 1;
			checkHit(x, y);
		}
		//controls button
		if ((MainMenu) && (mouseX <= (double)561.6 / (double)800) && (mouseX >= (double)238.4 / (double)800)
			&& (mouseY <= (double)383 / (double)500) && (mouseY >= (double)329 / (double)500)){
			MainMenu = false;
			ControlMenu = true;
			//ben
		}
		...
		//score button
		if ((MainMenu) && (mouseX <= (double)561.6 / (double)800) && (mouseX >= (double)238.4 / (double)800)
			&& (mouseY <= (double)299 / (double)500) && (mouseY >= (double)245 / (double)500)){
			ScoreBoard = true;
			MainMenu = false;
		}
		...
		//stage button
		if ((MainMenu) && (mouseX <= (double)561.6 / (double)800) && (mouseX >= (double)238.4 / (double)800)
			&& (mouseY <= (double)215 / (double)500) && (mouseY >= (double)161 / (double)500)){
			StageMenu = true;
			MainMenu = false;
		}
		...
		// play button
		if ((MainMenu) && (mouseX <= (double)561.6 / (double)800) && (mouseX >= (double)238.4 / (double)800)
			&& (mouseY <= (double)131 / (double)500) && (mouseY >= (double)77 / (double)500)){
			play = true;
			MainMenu = false;
			if (currentLevel == 0)
			{
		...
		//exit game button
		if ((MainMenu) && (mouseX <= (double)561.6 / (double)800) && (mouseX >= (double)238.4 / (double)800)
			&& (mouseY <= (double)467 / (double)500) && (mouseY >= (double)413 / (double)500)){
			DisplayExit = true;
			MainMenu = false;
			play = false;
		}
		...
		ymouse = y;
		//main screen button for score board
		if ((ScoreBoard) && (mouseX <= (double)561.6 / (double)800) && (mouseX >= (double)238.4 / (double)800)
			&& (mouseY <= (double)467 / (double)500) && (mouseY >= (double)413 / (double)500)){
			ScoreBoard = false;
			MainMenu = true;
		}
		...
		//main screen button for control menu
		if ((ControlMenu) && (mouseX <= (double)561.6 / (double)800) && (mouseX >= (double)238.4 / (double)800)
			&& (mouseY <= (double)467 / (double)500) && (mouseY >= (double)413 / (double)500)){
			ControlMenu = false;
			MainMenu = true;
		}

		//main screen button for stage menu
		if ((StageMenu) && (mouseX <= (double)561.6 / (double)800) && (mouseX >= (double)238.4 / (double)800)
			&& (mouseY <= (double)467 / (double)500) && (mouseY >= (double)413 / (double)500)){
			StageMenu = false;
			MainMenu = true;
		}

		//stage 3 button - stage menu
		if ((StageMenu) && (mouseX <= (double)561.6 / (double)800) && (mouseX >= (double)238.4 / (double)800)
			&& (mouseY <= (double)376.7 / (double)500) && (mouseY >= (double)311.26 / (double)500)){
			StageMenu = false;
			play = true;
			if (currentLevel == 0)
			{
		...
		//stage 2 button - stage menu
		if ((StageMenu) && (mouseX <= (double)561.6 / (double)800) && (mouseX >= (double)238.4 / (double)800)
			&& (mouseY <= (double)274.96 / (double)500) && (mouseY >= (double)209.62 / (double)500)){
			StageMenu = false;
			play = true;
			if (currentLevel == 0)
			{
		...
		//stage 1 button - stage menu
		if ((StageMenu) && (mouseX <= (double)561.6 / (double)800) && (mouseX >= (double)238.4 / (double)800)
			&& (mouseY <= (double)173.32 / (double)500) && (mouseY >= (double)107.98 / (double)500)){
			StageMenu = false;
			play = true;
			if (currentLevel == 0)
			{
...
// Ben Porter Used for rollover textures in menu
//--------------------------------------------------------------------------------------

void RolloverMouse(int x1, int y1){
	//controls button rollover

	double mouseX = ((double)x1 / (double)width);
	double mouseY = ((double)y1 / (double)height);
	if ((MainMenu) && (mouseX <= (double)561.6 / (double)800) && (mouseX >= (double)238.4 / (double)800)
		&& (mouseY <= (double)383 / (double)500) && (mouseY >= (double)329 / (double)500)){
	...
	//main screen rollover button for score board
	if ((ScoreBoard) && (mouseX <= (double)561.6 / (double)800) && (mouseX >= (double)238.4 / (double)800)
		&& (mouseY <= (double)467 / (double)500) && (mouseY >= (double)413 / (double)500)){
		scoreMainMenuRollover = true;
	}
	else{
		scoreMainMenuRollover = false;
	}

	//main screen rollover button for control menu
	if ((ControlMenu) && (mouseX <= (double)561.6 / (double)800) && (mouseX >= (double)238.4 / (double)800)
		&& (mouseY <= (double)467 / (double)500) && (mouseY >= (double)413 / (double)500)){
		controlMainMenuRollover = true;
	}
	else{
		controlMainMenuRollover = false;
	}

	//main screen rollover button for stage menu
	if ((StageMenu) && (mouseX <= (double)561.6 / (double)800) && (mouseX >= (double)238.4 / (double)800)
		&& (mouseY <= (double)467 / (double)500) && (mouseY >= (double)413 / (double)500)){
		stageMainMenuRollover = true;
	}
	else{
		stageMainMenuRollover = false;
	}
	//stage 3 button rollover
	if ((StageMenu) && (mouseX <= (double)561.6 / (double)800) && (mouseX >= (double)238.4 / (double)800)
		&& (mouseY <= (double)376.7 / (double)500) && (mouseY >= (double)311.26 / (double)500)){
		stage3Rollover = true;
	}
	else{
		stage3Rollover = false;
	}
...
void CreateBoundingBoxes()
{
	...
	//// Canteen block
	//cam.SetAABBMaxX(12, 2670.0);
	//cam.SetAABBMinX(12, 2608.0);
	//cam.SetAABBMaxZ(12, 41168.0);
	...
	////below are for the extended arts area
	////wall on the left courtyard entrance
	//cam.SetAABBMaxX(17, 2670);
	//cam.SetAABBMinX(17, 2608);
	//cam.SetAABBMaxZ(17, 50000);
	//cam.SetAABBMinZ(17, 42960);

	////wall on the right of courtyard
	//cam.SetAABBMaxX(18, 2608);
	//cam.SetAABBMinX(18, -17500);
	//cam.SetAABBMaxZ(18, 33400);
	//cam.SetAABBMinZ(18, 31500);

	////left side of staircase wall side close to bushcourt
	//cam.SetAABBMaxX(19, 2608);
	//cam.SetAABBMinX(19, 1328);
	//cam.SetAABBMaxZ(19, 41168);
	//cam.SetAABBMinZ(19, 40912);

	////right side of staircase wall side close to bush court
	////need to fix z min to an the back wall-fixed
	//cam.SetAABBMaxX(20, 2608);
	//cam.SetAABBMinX(20, 1648);
	//cam.SetAABBMaxZ(20, 39800);
	//cam.SetAABBMinZ(20, 33000);

	////left wall once you walk in
	//cam.SetAABBMaxX(21, 2608);
	//cam.SetAABBMinX(21, -18742);
	//cam.SetAABBMaxZ(21, 43500);
	//cam.SetAABBMinZ(21, 42960);

	////far wall away from bushcourt and right side of the staircase there
	//cam.SetAABBMaxX(22, -17242);
	//cam.SetAABBMinX(22, -18742);
	//cam.SetAABBMaxZ(22, 39800);
	//cam.SetAABBMinZ(22, 3400);

	////back wall of far off staircase
	//cam.SetAABBMaxX(23, -18742);
	//cam.SetAABBMinX(23, -18900);
	//cam.SetAABBMaxZ(23, 42960);
	//cam.SetAABBMinZ(23, 39800);

	////left side of far staircase in arts area
	//cam.SetAABBMaxX(24, -17242);
	//cam.SetAABBMinX(24, -18900);
	//cam.SetAABBMaxZ(24, 41168);
	//cam.SetAABBMinZ(24, 40912);
}
...
{	
	...
	for (int i = 0; i < 18 ; i ++)
	{
		//creates a flat plain at each location
		//format is (plain type, x min, x max, y min, y max, z min, z max)
	...
	for (int i = 0; i < 18; i++){
		...
		cam.SetPlains(FLAT_PLAIN, stepLength, stepLength + 42, step, step, 40150, 40150 + 684);
		step -= 40;
		stepLength -= 100;
	}
	*/
	...
	for (int i = 0; i < 18; i++)
	{
		...
		//format is (plain type, x min, x max, y min, y max, z min, z max)
		//what I think it does is create a plain for every instance of i
		cam.SetPlains(FLAT_PLAIN, stepLength, stepLength + 42, step, step, 40150, 40150 + 684);
		step -= 48.0;
		stepLength -= 142.0;
		if ((i + 3) % 5 == 0)
...
void DrawBackdrop()
{
	...
	CreateWalls(); // used by Tye to draw the couryard entrance
}
