...
Camera::Camera()
{
	...
	//https://freesound.org/people/OwlStorm/sounds/151230/
	grassStepSound = es->GetSound(es->Load("sounds/151230__owlstorm__grassy-footstep-1.wav"));
}
...
void Camera::MoveFB()
{
	...
	if (m_CollisionDetectionOn)
	{
		...
		if (!(m_colDetect.Collide(startX + m_lookX, m_y + m_lookY, startZ + m_lookZ)))
		{
			...
			if (m_z > 33540.5 && m_z < 40786.2 && m_x < 223.696 && m_x > -15509.7)
			{
				if (p % 20 == 0)
				{
					grassStepSound->Play();
	...
	std::cout << m_x << ", "<< m_y - 450 << ", " << m_z << std::endl;
}
